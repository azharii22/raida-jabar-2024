                ...
                <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ ++$i }}</td>
                            <td>{{ $item->judul }}</td>
                            <td><a href="{{ Storage::url('public/img/dokumentasi/cover/').$item->cover }}" target="_blank">
                                    <img class="rounded-circle header-profile-user" src="{{ isset($item->cover) ? asset(Storage::url('public/img/dokumentasi/cover/').$item->cover) : asset('/assets/images/users/avatar-icon.webp') }}" id="formrow-foto-input" width="50px" height="50px" alt="">
                                </a></td>
                            <td class="text-center">
                                <a href="{{ route('admin-dokumentasi-kegiatan.show', $item->id) }}" class="btn btn-light waves-effect waves-light btn-sm mr-2"> <i class="mdi mdi-eye"></i> View Photo Dokumentasi</a>
                                <a href="{{ route('addPhotos', $item->id) }}" class="btn btn-info waves-effect waves-light btn-sm mr-2"> <i class="bx bx-plus"></i> Add Photos</a>
                                <a href="{{ route('admin-dokumentasi-kegiatan.edit',$item->id) }}" class="btn btn-warning waves-effect waves-light btn-sm mr-2"> <i class="bx bx-pencil"></i> Edit</a>
                                <button type="button" class="btn btn-danger waves-effect waves-light btn-sm mr-2" data-bs-toggle="modal" data-bs-target="#modal-delete-{{ $item->id }}"> <i class="bx bx-trash"></i> Delete</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<!-- start modal edit -->
@foreach ($data as $i => $item)
<div class="modal fade" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true" id="modal-edit-{{ $item->id }}">
